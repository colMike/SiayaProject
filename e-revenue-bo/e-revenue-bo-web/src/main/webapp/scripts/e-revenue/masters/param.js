...
angular.module('app.param', []).controller("paramCtrl", ["$scope", "$filter", "paramSvc","$paramValid", "$rootScope", "blockUI", "logger", "$location", function ($scope, $filter,paramSvc, $paramValid, $rootScope, blockUI, logger, $location) {
    ...
    /**
     * gets the list of paramType
     */
    $scope.paramTypes=[{id:1,paramType:'ALPHA'},
                       {id:1,paramType:'NUM'}]
    
    /**
     * gets the details of selected param
     * @parma parma-object of param detail
     */
    $scope.editParam = function (param) {
        ...
        $scope.paramType=param.paramType;
        $scope.active = param.active;
        
    };
    
    /**
     *  initialize the form for creating the new param
     */
    ...
    /**
     * cancels the operation of param creation
     */
    ...
    $scope.updParam = function () {
        ...
        if (!$paramValid($scope.paramType)) {
            logger.logWarning("Opss! You may have skipped specifying the Param type. Please try again.")
            return false;
        }
        ...
        param.paramType = $scope.paramType;
    ...
    paramDeviceSvc.UpdParam = function (device) {
        return $http({
            url: '/erevenue/rest/service/updParam',
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            data: device
