 ...
 */
'use strict';
angular.module('app.rptByGateService', []).controller("rptByGateServiceCtrl", ["$scope", "$filter", "serviceTxnSvc","entryPointSvc","issueDeviceSvc","memStatementSvc","$serviceTxnValid", "$rootScope", "blockUI", "logger", "$location","$http","$window", function ($scope, $filter, serviceTxnSvc,entryPointSvc,issueDeviceSvc,memStatementSvc,$serviceTxnValid, $rootScope, blockUI, logger, $location,$http,$window) {
...
//       }
       ...
       serviceTxnSvc.GateServiceDetailedTxn(service).success(function (response) {
       	console.log(response);
           return $scope.userTxns = response, $scope.searchKeywords = "", $scope.filteredUserTxns = [], $scope.row = "", $scope.select = function (page) {
               var end, start;
               return start = (page - 1) * $scope.numPerPage, end = start + $scope.numPerPage, $scope.currentPageUserTxns = $scope.filteredUserTxns.slice(start, end)
   ...
   $scope.exportPdfReport = function(mem) {
		...
		$scope.url = '/kcmc/reports?type=RPGS&eType=P&serId='+serviceId+'&subSerId='+subSerId+'&FrDt='+ FromDt + '&ToDt=' + ToDt + '&gateId=' + gateId ;
	...
	$scope.exportExcelReport = function(mem) {
		...
		if(gateId==null){
			...
			$scope.url = '/kcmc/reports?type=RPGS&eType=E&serId='+serviceId+'&subSerId='+subSerId+'&FrDt='+ FromDt + '&ToDt=' + ToDt;
	}
