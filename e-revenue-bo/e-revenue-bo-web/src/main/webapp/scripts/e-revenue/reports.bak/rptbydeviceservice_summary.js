 ...
 */
'use strict';
angular.module('app.rptDeviceServiceSummary', []).controller("rptDeviceServiceSummaryCtrl", ["$scope", "$filter", "serviceTxnSvc","entryPointSvc","issueDeviceSvc","deviceTxnSvc","memStatementSvc","$devserviceValid", "$rootScope", "blockUI", "logger", "$location","$http","$window", function ($scope, $filter, serviceTxnSvc,entryPointSvc,issueDeviceSvc,deviceTxnSvc,memStatementSvc,$devserviceValid, $rootScope, blockUI, logger, $location,$http,$window) {
...
//       }
	   ...
	   $scope.userTxns = [], $scope.searchKeywords = "", $scope.filteredUserTxns = [], $scope.row = "", $scope.agentEditMode = false; $scope.previewServices=false;
       var device ={};
       device.parentSerId=$scope.serviceSelect;
       device.subSerId=$scope.subSerSelect;
       device.fromDate=$scope.fromDt;
       device.toDate=$scope.toDt;
       device.gateId=$scope.gateSelect;
       device.deviceId=$scope.deviceSelect;
       deviceTxnSvc.ServicePerDeviceSummary(device).success(function (response) {
          	console.log(response);
              return $scope.userTxns = response, $scope.searchKeywords = "", $scope.filteredUserTxns = [], $scope.row = "", $scope.select = function (page) {
                  var end, start;
                  return start = (page - 1) * $scope.numPerPage, end = start + $scope.numPerPage, $scope.currentPageUserTxns = $scope.filteredUserTxns.slice(start, end)
   ...
   $scope.exportPdfReport = function(mem) {
		...
		$scope.url = '/kcmc/reports?type=RPDSsum&eType=P&serId='+serviceId+'&subSerId='+subSerId+'&FrDt='+ FromDt + '&ToDt=' + ToDt +  '&gateId=' +gateId +  '&deviceId=' +deviceId;
	...
	$scope.exportExcelReport = function(mem) {
		...
		if(gateId==null){
			...
			$scope.url = '/kcmc/reports?type=RPDSsum&eType=E&serId='+serviceId+'&subSerId='+subSerId+'&FrDt='+ FromDt + '&ToDt=' + ToDt +  '&gateId=' +gateId +  '&deviceId=' +deviceId;
	}

}])
.factory('$devserviceValid', function () {
